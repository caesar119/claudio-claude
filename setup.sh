...
#   claudio code [worker|dir]            open Claude Code in tmux
#   claudio hire <name> <dir> [voice]    hire a worker for a project
#   claudio fire <name> / claudio team   manage the team
#   claudio voz on|off|status|test       Claude speaks its replies
#   claudio start|stop|status|log|fg     control the voice daemon
set -u
REPO="$DIR"
case "\${1:-}" in
    code|start|stop|status|log|fg|tmux|voz|voice|hire|fire|team|equipo)
        exec bash "\$REPO/claudio.sh" "\$@"
        ;;
    *)
        echo "Usage: claudio {code [worker|dir]|hire <name> <dir> [voice]|fire <name>|team|voz on|off|status|test|start|stop|status|log|fg}"
        exit 1
        ;;
esac
EOF
